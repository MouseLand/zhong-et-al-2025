...
def lick_raster_plot(ax, lick, show_reward=1, show_firstLick=0, title='', xlm=[0, 60], tcolor='k'):
    ...
    utils.fmt(ax, xtick=[[0,20,40,60], [0,2,4,6]], ytick=[[0, ntrials]], tcolor=tcolor,
          ylabel='tirals', xlabel='position (m)', title=title, xlm=xlm, ylm=[0, ntrials], y_invert=1) 

def sup_perf_plot(ax, perf, title='', xlm=[-0.3, 1.3]):
    r = perf['u_sem']
    ...
    utils.fmt(ax, xtick=[np.arange(len(perf['stimuli'])), perf['stimuli']], ytick=[[0, 0.5, 1], [0, 50, 100]],
          ylabel='anticipatory licking (%trials)', title=title, xlm=xlm, ylm=[0, 1])
...
def plot_rewPred_neu_frac(ax, root, fn0='sup_train1_before_learning_rew_frac.npy', fn1='sup_train1_after_learning_rew_frac.npy', col='k'):
    # load reward neurons fractions
    rew_bef = np.load(os.path.join(root, 'process_data', fn0), allow_pickle=1).item()
    rew_aft = np.load(os.path.join(root, 'process_data', fn1), allow_pickle=1).item()
    plot_frac(ax, rew_bef['value'][:, :, 0], rew_aft['value'][:, :, 0], col=col)
    utils.fmt(ax, ylm=[0, 0.12])
    for t,txt in enumerate(['V1', 'mHV', 'lHV', 'aHV']):
        ax.text(0.1 + 0.25*t, 0.95, txt, transform=ax.transAxes)

def plot_test1_CD_proj(ax, dat):
    for i, iar in enumerate(['V1', 'mHV', 'lHV', 'aHV']):
        for j, jc in enumerate(['r', 'm', 'b', 'c']):
            ax[i].plot(dat['proj_tr_mean'][:, i, j].T, color=jc, lw=1, alpha=0.3) 
            u = np.nanmean(dat['proj_tr_mean'][:, i, j], axis=0)
            sem = np.nanstd(dat['proj_tr_mean'][:, i, j], axis=0, ddof=1)/np.sqrt(dat['proj_tr_mean'].shape[0])
            ax[i].plot(np.arange(len(u)), u, color=jc, lw=1.5)  # Mean line
            ax[i].fill_between(np.arange(len(u)), u - sem, u + sem, color=jc, alpha=0.3)  # Shaded STD area            
        ax[i].axvline(dat['pos_from_prev'], linestyle='--', lw=0.5)
        ax[i].axvline(dat['pos_from_prev']+40, linestyle='--', lw=0.5)
        utils.fmt(ax[i], ylabel='projection (a.u.)', xlabel='position (m)',
                 xtick=[np.arange(7)*10 + dat['pos_from_prev'], np.arange(7)],
                 xlm=[0, 70], ylm=[-1.2, 1.2], title=iar)
...
def peak_position_scatter_plot(ax, dat, mname='VR2', arname='mHV', stim_sort='leaf1'):
    ...
    for i, ic in enumerate([0, 1, 3, 2]):
        ...
        if tn[i]==stim_sort:
            ...
            xn = 'position (m)'
        utils.fmt(ax[i], title='$r=%.2f$'%(r), tcolor=cols[i], xlabel=xn, ylabel=yn)     
...
def test1_peak_pos_scatter_plot(ax, dat, mname='VR2', arname='mHV', stim_sort='leaf1', vmax = 1):
    ...
    for i, ic in enumerate([0, 1, 3, 2]):
        ...
        if i>0:
            xn, yn = '', ''         
        utils.fmt(ax[i], title=tn[i], tcolor=cols[i], y_invert=1, xlabel=xn, ylabel=yn)  
...
def plot_exampe_stimSel_single_neuron(ax, root='', stim=0, neu_n=27, vmax=3, cols=['r', 'b']):
    ...
    ax[0].text(0.4, 0.8, 'stim1', color=cols[0], transform=ax[0].transAxes)
    ...
    ax[1].text(0.4, 0.8, 'stim2', color=cols[1], transform=ax[1].transAxes)
...
def plot_exampe_stimSel_population(ax, root='', stim=0,  vmax=2, cols=['r', 'b']):
    ...
    ax[0].axvline(40, linestyle='--', lw=0.5, color='k')
    utils.fmt(ax[0], xlabel='position (m)', ylabel='neurons', y_invert=1, xtick=[[0, 20, 40, 60], [0, 2, 4, 6]], title='stim1', tcolor=cols[0], ytick=[[]])
    ax[1].imshow(stim2_tr, cmap='gray_r', vmin=0, vmax=vmax)
    ax[1].axvline(40, linestyle='--', lw=0.5, color='k')
    utils.fmt(ax[1], y_invert=1, xtick=[[0, 20, 40, 60], [0, 2, 4, 6]], title='stim2', tcolor=cols[1], ytick=[[]])         
...
def brain_coordinate(ax, fs=None, lw=1):
    ...
    AX(ax,xlm=[-0.7,0.7],ylm=[-1.2,1.2],axis_off='off')              